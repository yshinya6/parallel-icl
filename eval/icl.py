...
def eval_model(args):
    ...
    make_input_zs = get_make_input_func(model_name)
    ...
    for line in tqdm(questions, total=len(questions)):
        ...
        if len(contexts) > 0:
            model_kwargs = make_input(processor, instruction, contexts, image_path, question).to(
                model.device, dtype=torch.bfloat16
            )
        else:
            model_kwargs = make_input_zs(processor, image_path, question).to(model.device, dtype=torch.bfloat16)
