...
from modeling.gemma3 import reduce_visual_tokens as gemma3_reduction
from modeling.idefics3 import reduce_visual_tokens as idefics3_reduction
from modeling.llava import reduce_visual_tokens as llava_token_reduction
from modeling.llava_onevision import reduce_visual_tokens as llava_ov_reduction
from modeling.qwen2_5_vl import reduce_visual_tokens as qwen_2_5_token_reduction
from utils.output import GenerateDecodingOutput
...
def _compile_logits(logits: torch.Tensor, scores: torch.Tensor, strategy: str, temperature=1.0):
    match strategy:
        case "uniform":
            return logits.mean(dim=0).unsqueeze(0)
        case "uniform_poe":
            probs = F.softmax(logits, dim=-1)
            return probs.prod(dim=0).unsqueeze(0)
        case "score":
            return (scores.unsqueeze(-1) * logits).sum(dim=0).unsqueeze(0)
        case "score_poe":
            probs = F.softmax(logits, dim=-1)
            return (scores.unsqueeze(-1) * probs).prod(dim=0).unsqueeze(0)
        case "moe":
            probs = F.softmax(logits, dim=-1)
            return (scores.unsqueeze(-1) * probs).sum(dim=0).unsqueeze(0)
        case "score_pow":
            probs = F.softmax(logits, dim=-1)
            return (probs ** scores.unsqueeze(-1)).prod(dim=0).unsqueeze(0)
        case "entropy":
            entropies = _calculate_entropy(logits)
            weights = F.softmax(-entropies / temperature, dim=-1)
            return (weights.unsqueeze(-1) * logits).sum(dim=0).unsqueeze(0)
        case _:
